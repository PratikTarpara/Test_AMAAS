...
# Add the Slicer directory to the system path
slicer_directory = os.path.normpath(os.path.join(os.path.dirname(__file__), '../../../../Slicer'))
sys.path.append(slicer_directory)

# Import the convert_stl_to_gcode module
try:
    from slicer import convert_stl_to_gcode, extract_gcode_info
except ImportError:
    print(f"Failed to import from slicer.py: {e}")
  
# Add the Upload_Gcode directory to the system path
upload_gcode_directory = os.path.normpath(os.path.join(os.path.dirname(__file__), '../../../../Upload_Gcode'))
sys.path.append(upload_gcode_directory)

# Attempt to import the upload_and_print_gcode function from Upload_Print.py
try:
    from Upload_Print import upload_and_print_gcode
    print("Successfully imported upload_and_print_gcode from Upload_Print.py")
except ImportError as e:
    print(f"Failed to import upload_and_print_gcode from Upload_Print.py: {e}")
...
class SliceFile(AState):
    ...
    def actions(self) -> None:
        ...
        # stl_file_path
        stl_file_path = os.path.normpath(os.path.join(script_dir, '../../../../DesignFiles/test.stl'))
        # output_folder
        output_folder = os.path.normpath(os.path.join(script_dir, '../../../../Gcode'))
        # layer_height
        message = self.retrieve("callForProposal")
        self.save_in_message(message)
        submodel_sr = message["interactionElements"][0][0]
        SR_Layer_Height = submodel_sr['submodelElements'][1]['value'][0]['value']
        layer_height = SR_Layer_Height
        # gcode_file_name
        gcode_file_name = message["frame"]["conversationId"]
        # config_file_path
        config_file_path = os.path.normpath(os.path.join(os.path.dirname(__file__), '../../../../Slicer/config.ini'))
        
        # Convert STL to G-code
        output_gcode_file = convert_stl_to_gcode(stl_file_path, output_folder, layer_height, gcode_file_name, config_file_path)
        self.push("GcodeFilePath", output_gcode_file)
...
class AcceptApproval(AState):
    ...
    def initialize(self):
        # Gaurd variables for enabling the transitions
        self.StartPrinting_Enabled = True
            
    
    def actions(self) -> None:
        Gcode_File = self.retrieve("GcodeFilePath")
        relative_path = Gcode_File
        printer_url =  "http://169.254.32.166:10088"
        upload_and_print_gcode(relative_path, printer_url)
        
    
    def transitions(self) -> object:
        if (self.StartPrinting_Enabled):
            return "StartPrinting"
        
class StartPrinting(AState):
    
    def initialize(self):
        # Gaurd variables for enabling the transitions
        self.FinishPrinting_Enabled = True
    ...
    def transitions(self) -> object:
        if (self.FinishPrinting_Enabled):
            return "FinishPrinting"
                
class FinishPrinting(AState):
